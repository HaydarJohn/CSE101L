...
return 0;
    ...
    public static char[] grade(int[] students) 
    {
        ...
        int maxScore = getMax(students);

        // Assigning Letter note

        char returnChar[] = new char[students.length];

        for(int i = 0; i < students.length;i++)
        {
            if(maxScore-10<=students[i])
            {
                returnChar[i] = 'A';
            }
            else if(maxScore-20<=students[i])
            {
                returnChar[i] = 'B';
            }
            else if(maxScore-30<=students[i])
            {
                returnChar[i] = 'C';
            }
            else if(maxScore-40<=students[i])
            {
                returnChar[i] = 'D';
            }
            else
    ...
    public static int random(int start, int end, int... exclude) {
         ...
         */
         ...
         for(int i =0;i<exclude.length;i++)
         {
            if(num == exclude[i])
            {
                return random(start, end, exclude);
            }
         }
         return num;
    }
